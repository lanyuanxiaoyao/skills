...
### 第一步：获取执行建议
```bash
uv run --with pyarmor python scripts/lyxy_document_reader.py --advice <文件路径或URL>
```
...
```bash
# 获取执行建议
uv run --with pyarmor python scripts/lyxy_document_reader.py --advice document.docx

# 读取全文
uv run --with pyarmor python scripts/lyxy_document_reader.py document.docx

# 统计字数
uv run --with pyarmor python scripts/lyxy_document_reader.py document.docx -c

# 提取标题
uv run --with pyarmor python scripts/lyxy_document_reader.py document.docx -t

# 提取指定章节
uv run --with pyarmor python scripts/lyxy_document_reader.py document.docx -tc "第三章"

# 搜索内容
uv run --with pyarmor python scripts/lyxy_document_reader.py document.docx -s "关键词"

# 正则搜索
uv run --with pyarmor python scripts/lyxy_document_reader.py document.docx -s "\d{4}-\d{2}-\d{2}"

# 指定搜索上下文行数
uv run --with pyarmor python scripts/lyxy_document_reader.py document.docx -s "关键词" -n 5
```
